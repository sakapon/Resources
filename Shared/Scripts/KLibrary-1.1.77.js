...
(function (KLibrary) {
    ...
    function isArray(obj) {
        ...
        return obj != null && obj.length != null && !(obj instanceof Function);
    }
    ...
    Number.prototype.format = function (_format) {
        ...
        if (_format == null) _format = "N";

        var number = this;
        return _format.replace(/([nNxX])(\d*)/g,
            function (s, g0, g1) { return formatNumber(number, g0, g1 == "" ? 0 : parseInt(g1)); });
    };

    function formatNumber(number, c, length) {
        switch (c) {
            case "n":
            case "N": return number.toString().replace(/\d(?=(\d{3})+(\.\d+)?$)/g, "$0,");
            case "x": return number.toString(16).toLowerCase();
            case "X": return number.toString(16).toUpperCase();
            default: return c;
        }
    }
    ...
    (function (ArrayEx) {
        ...
        Array.prototype.all = function (predicate) {
            ...
            /// </summary>
            /// <param name="predicate" type="Function">要素が条件を満たしているかどうかを試す関数。</param>
            /// <returns type="Boolean" />
        ...
        Array.prototype.groupBy = function (toKey, toValue) {
            ...
            /// </summary>
            /// <param name="toKey" type="Function">キーを抽出する関数。</param>
            /// <param name="toValue" type="Function">値を抽出する関数。</param>
            /// <returns type="Array" />

            var d = this.groupByToDictionary(toKey, toValue);
            var a = [];
            for (var p in d) {
                a.push({ key: p, value: d[p] });
            }
            return a;
        };
        ...
        Array.prototype.groupByToDictionary = function (toKey, toValue) {
            ...
            var d = {};
            for (var i = 0; i < this.length; i++) {
                var key = toKey(this[i]);
                var value = toValue(this[i]);
                if (d[key]) {
                    d[key].push(value);
                } else {
                    d[key] = new Array(value);
                }
            }
            return d;
        };
        ...
        Array.prototype.select = function (selector) {
            ...
            ///     要素を新たな値に射影します。
            /// </summary>
            /// <param name="selector" type="Function">要素を変換する関数。</param>
            /// <returns type="Array" />

            var a = [];
            for (var i = 0; i < this.length; i++) {
                a.push(selector(this[i], i));
            }
            return a;
        };
        ...
        Array.prototype.where = function (predicate) {
            ...
            ///     要素をフィルター処理します。
            /// </summary>
            /// <param name="predicate" type="Function">要素をフィルターする関数。</param>
            /// <returns type="Array" />
    ...
    function toEntity(obj) {
        ...
        return !(obj instanceof Array) ?
            new Entity(obj) :
            obj.select(function (x) { return new Entity(x); });
    };
    ...
    (function (IO) {
        ...
        function setCookie(key, value, days, domain, path, isSecure) {
            ...
            var cookie = format("{0}={1};", key, escape(value));
            if (days) {
...
(function ($) {
    $.uploadJSON = function (url, data, method) {
        /// <summary>
